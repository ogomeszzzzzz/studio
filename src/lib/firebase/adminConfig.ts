...
console.log('--- [ADMIN SDK INIT - ATTEMPTING INITIALIZATION V12] ---');
...
const expectedProjectId = "ecommerce-db-75f77"; // Your target project ID
...
if (!admin || !admin.credential || typeof admin.credential.cert !== 'function' || typeof admin.initializeApp !== 'function') {
  ...
  if (serviceAccountString && serviceAccountString.trim() !== '{}' && serviceAccountString.trim() !== '') {
    ...
    try {
      ...
      console.log(`[Admin SDK] Successfully parsed JSON from FIREBASE_ADMIN_SDK_CONFIG. Project ID: ${serviceAccountCredentials?.project_id}`);
    } catch (e: any) {
      ...
      if (!credential || Object.keys(credential).length === 0) {
        ...
        try {
          ...
          if (adminApp) {
            ...
            // console.log(`[Admin SDK] adminApp.options:`, JSON.stringify(adminApp.options, null, 2));
...
if (!adminApp && !adminSDKInitializationError) {
  adminSDKInitializationError = 'Firebase Admin SDK app could not be initialized, and no specific error was caught during the credential loading/parsing attempt.';
  console.error(`[Admin SDK Init Error] ${adminSDKInitializationError}`);
}

export const adminAuth = adminApp ? adminApp.auth() : null;
export const adminFirestore = adminApp ? adminApp.firestore("ecom") : null; // Ensure connecting to "ecom" database
...
console.log('--- [ADMIN SDK INIT END V12] ---');
