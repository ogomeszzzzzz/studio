...
console.log('[Firebase Client Config V2] Reading environment variables for project setup:');
...
console.log('NEXT_PUBLIC_FIREBASE_PROJECT_ID:', process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID); // Should be ecommerce-db-75f77
...
if (!firebaseConfig.projectId) {
  ...
  console.error(`[Firebase Client Config V2] ${firestoreClientInitializationError}`);
...
if (!firestoreClientInitializationError) {
  if (!getApps().length) {
    console.log('[Firebase Client Config V2] No Firebase apps initialized. Initializing new app...');
    try {
      clientApp = initializeApp(firebaseConfig);
      console.log(`[Firebase Client Config V2] Firebase app initialized successfully. Project ID: ${clientApp.options.projectId}`);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.error(`[Firebase Client Config V2] Error initializing Firebase app:`, errorMessage);
    ...
    clientApp = getApp();
    console.log(`[Firebase Client Config V2] Firebase app already initialized. Using existing app. Project ID: ${clientApp.options.projectId}`);
  ...
  console.error('[Firebase Client Config V2] Firebase app not initialized due to configuration errors listed above.');
}
...
if (clientApp && !firestore) {
    console.error('[Firebase Client Config V2] Client app initialized, but Firestore instance could not be obtained.');
    if (!firestoreClientInitializationError) {
        firestoreClientInitializationError = 'Firestore instance could not be obtained even though Firebase App seems initialized.';
    }
} else if (firestore) {
    console.log(`[Firebase Client Config V2] Firestore instance obtained for (default) database of project: ${clientApp?.options.projectId}.`);
} else {
    console.log('[Firebase Client Config V2] Firestore instance is null. This is expected if clientApp is null or initialization failed.');
}
